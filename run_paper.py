...
End-to-end runner for the Nocturne Atlas in the cleaner *paper* register
(ivory engraving paper rather than aged parchment). Generates two paired
plates and reports Music-of-the-Spheres ring statistics.
...
def run_pair(chart_path: Path, audio_path: Path, label: str,
    ...
    # 4) Source chart -> paper
    src_chart = Image.open(chart_path).convert("RGBA")
    src_p = paper_chart_overlay(src_chart, seed=11, with_atlas=True)
    ...
    chart_p = paper_chart_overlay(chart_img, seed=53)
    ...
    paper_waveform(audio_path, wave_path, width=2400, height=900, seed=29)
